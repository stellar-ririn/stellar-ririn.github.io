                ...
                if (useDictionary) {
                    ...
                    sortedDynamicWords.forEach(word => {
                        if (word) {
                            ...
                            dynamicPlaceholders[placeholder] = word;
...
${modifiedText}
    ...
    exportPdfBtn.addEventListener('click', async function () {
        M.toast({ html: 'PDF生成を開始します。少し時間がかかります…', displayLength: 3000 });
        ...
        try {
            ...
            const comment = document.getElementById('output-comment').value;
            const originalText = document.getElementById('input-area').value;
            const modifiedText = document.getElementById('output-area').innerText;
            ...
            // フォントのURL (Google FontsのNoto Sans JP)
            const fontUrl = 'https://fonts.gstatic.com/s/notosansjp/v60/-F62fjtqLzI2JPCgQBnw7HFowA.otf';
            ...
            // ヘッダー情報の描画
            page.drawText(`[題名]: ${title}`, { x: margin, y, font: customFont, size: fontSize + 4 });
            y -= 20;
            page.drawText(`[コメント]: ${comment}`, { x: margin, y, font: customFont, size: fontSize });
            y -= 20;
            page.drawText(`[キーワード]: ${keywords}`, { x: margin, y, font: customFont, size: fontSize });
            y -= 30;
            ...
            for (const part of diff) {
                const color = part.added ? rgb(0, 0.6, 0) : part.removed ? rgb(0.8, 0, 0) : rgb(0, 0, 0);
                const prefix = part.added ? '+ ' : part.removed ? '- ' : '  ';
                const lines = (prefix + part.value).split('\n');
                ...
                for (const line of lines) {
                    ...
                    page.drawText(line, { x: margin, y, font: customFont, size: fontSize, color });
                    y -= (fontSize + 2);
                }
            }
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `${title || 'diff'}.pdf`;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            URL.revokeObjectURL(url);
            M.toast({ html: 'PDFの生成が完了しました。' });

        } catch (error) {
            console.error('PDFの生成に失敗しました:', error);
            M.toast({ html: 'PDFの生成に失敗しました。フォントの読み込みに問題があるかもしれません。' });
        }
    });
});
